...
public class Day14Tests
{
    ...
    [Test]
    public async Task Solves_puzzle_input_for_part_1()
    {
    ...
    [Test]
    public async Task Solves_puzzle_input_for_part_2()
    {
        var sut = new Day14(await File.ReadAllTextAsync("Day14.txt"), 101, 103);

        var safetyFactorAtSecond = new Dictionary<int, int>();

        for (int i = 1; i <= 10000; i++)
        {
            sut.Tick();
            safetyFactorAtSecond.Add(i, sut.CalculateSafetyFactor());
        }

        int secondWithLowestSafetyFactor = safetyFactorAtSecond.OrderBy(kvp => kvp.Value).First().Key;

        await Assert.That(secondWithLowestSafetyFactor).IsEqualTo(6771);
    }
}